...
class Condensor:
    ...
    def list_condensor(self, batch_id: str | None = None) -> list[dict]:
        ...
        for g in groups.values():
            ...
            g["total"] = total
            rate_map = dbm.get_commission_rates_for_physician(g["physician_id"])
            billed = {
                "Ultrasound": g["ultrasound"],
                "Laboratory": g["laboratory"],
                "X-ray": g["x-ray"],
                "Nursing & Procedures": g["nursing_and_procedures"],
                "Consultation": g["consultation"],
                "Other": g["other"],
            }
            _cats, amount = dbm.category_commission_breakdown(billed, rate_map)
            g["commision_amount"] = amount
            g["commision_percent"] = round(amount / total * 100.0, 2) if total else 0.0
            condensed.append(g)
